...
- 把本文件所在目录视为源码仓库根目录；不要假设调用时的当前目录就是仓库根目录。
...
- 涉及升级、回滚或卸载时，先阅读 [安装、升级与回滚](docs/install-upgrade-rollback.md)。
- 涉及 iOS 构建、签名或真机部署时，先阅读 [iOS 开发说明](ios/MimiRemote/README.md)。
...
在新建的临时目录中下载目标归档和同一 Release 的 `checksums.txt`，严格校验 SHA-256 后再解压。使用 [安装、升级与回滚](docs/install-upgrade-rollback.md) 中的完整下载命令，不要跳过校验。
...
若本 Skill 来自完整仓库，直接使用当前仓库。若只有 Skill 而没有源码：
...
1. 按 [安装、升级与回滚](docs/install-upgrade-rollback.md) 创建权限为用户私有的配置备份。
...
仅在用户明确要求后阅读 [Claude bridge 架构](docs/claude-bridge-architecture.md)，确认用户理解该通道仍是实验功能。要求 Rust/Cargo、Claude Code headless 环境和 `alleycat-claude-bridge >= 0.2.1`。
...
- 产品概览和快速开始：[README.zh-CN.md](README.zh-CN.md)
- 安装、升级、停止、回滚和 Linux 细节：[docs/install-upgrade-rollback.md](docs/install-upgrade-rollback.md)
- iOS 构建、真机部署和验收：[ios/MimiRemote/README.md](ios/MimiRemote/README.md)
- Codex app-server 协议边界：[docs/codex-protocol-support.md](docs/codex-protocol-support.md)
- Tailscale 运维：[docs/tailscale-peer-relay-ops.md](docs/tailscale-peer-relay-ops.md)
- 支持与去敏要求：[docs/support.md](docs/support.md)
