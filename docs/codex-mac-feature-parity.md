...
| Git diff 审查 | Git 面板可从 Inspector 或工作区卡片/顶栏直接打开，显式绑定目标目录，不会误用当前会话 cwd。完整状态与摘要都会返回 `ahead`、`behind`、`upstream`；回合结束后只对用户已经打开过的 Git 缓存按目录防抖刷新。Quick Publish 有 upstream 时尊重现有跟踪关系，无 upstream 才建立 `origin/<branch>`；落后远端时在 add/commit 前以 409 阻断。原有 diff stat、staged/unstaged patch、文件/hunk stage、unstage、revert、commit、push、草稿 PR、PR 状态和本地 hunk review note 保持可用。 | 仍缺 pull/rebase/merge 冲突处理，以及 GitHub Review API 级别的 inline comment 同步。 | 移动端暂不增加一键 pull/rebase；分支落后或分叉时明确阻断快捷发布，让用户在 Mac 端完成同步和冲突决策。 |
