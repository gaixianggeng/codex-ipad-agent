...
Download the versioned `Mimi-Remote-Setup-*.exe`, `.sha256`, and `.metadata.json` files from [GitHub Releases](https://github.com/gaixianggeng/codex-ipad-agent/releases/latest). Always verify the SHA-256. A public Release must report `authenticode-pfx` in `metadata.json` and an Authenticode status of `Valid`; missing signing credentials fail the release instead of publishing an unsigned installer:
...
Unsigned installers are limited to local or Nightly snapshots and are not public Release assets.
...
The Windows installer and Mac DMG already include a compatible bridge next to `agentd`; public Windows releases and the notarized Mac DMG preserve Authenticode and Developer ID code identity. Unsigned Windows builds are only local or Nightly snapshots. Do not install a second copy with Cargo for those setups. Install the bridge from source only for Homebrew, Linux, or standalone development:
