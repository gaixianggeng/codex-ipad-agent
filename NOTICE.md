...
iOS 工作区中的 Codex 运行时图标取自 OpenAI 官方 macOS 应用资源包中的 `icon-codex-light.png` 和 `icon-codex-dark-color.png`，仅按 iOS 显示倍率等比缩小，用于帮助用户识别其主动选择的 Codex 运行时。该图标及相关商标归 OpenAI 所有，不代表 OpenAI 对本项目的赞助或背书。
